...
// Include DB connection only
include("../assets/php/connections.php"); // Ensure this file contains your $login_connections
...
// Fetch user_id from the profile_tbl using the session's username
...
// Handling form submission to save or update a journal note
...
if (!is_dir($uploadDir)) {
    mkdir($uploadDir, 0777, true); // Creates the folder with full permissions
}

// Handle file upload
$attachmentPath = 'assets/img/' . basename($fileName);
if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($fileType, $allowedTypes) && $fileSize <= $maxFileSize) {
        ...
        if (move_uploaded_file($fileTmpPath, $filePath)) {
            $attachmentPath = 'assets/img/' . basename($fileName); // Save relative path for browser access
        } else {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'update') {
        ...
        $updateQuery = "UPDATE journal_tbl SET title = ?, description = ?, mood = ?, updated_at = ?, attachment_path = ? WHERE journal_id = ? AND user_id = ?";
        ...
        $created_at = date("F j | h:i A");

        $insertQuery = "INSERT INTO journal_tbl (user_id, title, description, mood, created_at, attachment_path) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $login_connections->prepare($insertQuery);
        $stmt->bind_param("isssss", $user_id, $title, $description, $mood, $created_at, $attachmentPath);
        $stmt->execute();
    }
...
$query = "SELECT journal_id, title, description, mood, created_at, updated_at, attachment_path FROM journal_tbl WHERE user_id = ?";
...
// Include header only after all headers have been sent
...
<head>
    ...
    background-position: center;      /* Keeps it centered on all screen sizes */
    background-attachment: fixed;     /* Optional: makes the background stay while scrolling */
    background-color: #f5f5f5;        /* Fallback in case image doesn't load */
}
...
<body class="bg-gray-100">
    <div class="popup-box flex items-center justify-center min-h-screen">
      <div class="popup rounded-lg shadow-lg w-full h-full max-w-3xl p-10">
          <div class="content w-full h-full sm:rounded-lg sm:max-w-3xl sm:mx-auto sm:my-auto sm:shadow-lg sm:p-10 p-6 bg-white overflow-y-auto">
                ...
                <div class="row attachment mb-4">
                    <label class="block text-sm font-medium text-gray-700">Attach a File</label>
                    <input type="file" name="attachment" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500 focus:border-blue-500 p-2">
                </div>

                    <div class="row title mb-4">
                        <label class="block text-sm font-medium text-gray-700">Title</label>
                        <input type="text" name="title" spellcheck="false" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500 focus:border-blue-500 p-2">
                    </div>
                    <div class="row mood mb-4">
                        <label class="block text-sm font-medium text-gray-700">Mood</label>
                        <div class="mood-options flex flex-wrap">
                            <input type="hidden" name="mood" id="selected-mood" required>
                            <label class="mood-happy cursor-pointer mr-2" onclick="changeMoodColor(this, 'Happy')">
                                <span class="text-lg">😊 Happy</span>
                            </label>
                            <label class="mood-sad cursor-pointer mr-2" onclick="changeMoodColor(this, 'Sad')">
                                <span class="text-lg">😢 Sad</span>
                            </label>
                            <label class="mood-excited cursor-pointer mr-2" onclick="changeMoodColor(this, 'Excited')">
                                <span class="text-lg">😆 Excited</span>
                            </label>
                            <label class="mood-angry cursor-pointer mr-2" onclick="changeMoodColor(this, 'Angry')">
                                <span class="text-lg">😡 Angry</span>
                            </label>
                            <label class="mood-relaxed cursor-pointer" onclick="changeMoodColor(this, 'Relaxed')">
                                <span class="text-lg">😌 Relaxed</span>
                            </label>
                        </div>
                    </div>

                    <div class="row description mb-4">
                        <label class="block text-sm font-medium text-gray-700">Description</label>
                        <textarea name="description" spellcheck="false" required     class="mt-1 block w-full h-52 border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500 focus:border-blue-500 p-2 resize-none"></textarea>
                    </div>
                    <input type="hidden" name="action" value="add">
                    <input type="hidden" name="noteId" value="">
                    <button type="submit" class="w-full bg-blue-500 text-white font-semibold py-2 rounded-md hover:bg-blue-600 transition duration-200">Add Note</button>
                </form>
            </div>
        </div>
    </div>

    <div class="view-popup-box fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" style="display: none;">
    <div class="view-popup bg-gradient-to-br from-gray-400 to-blue-400 rounded-lg shadow-lg max-w-md w-full p-6">
        <div class="content">
   ...
   <p id="view-description" class="text-gray-700 mb-4"></p>
                <span id="view-mood" class="inline-block bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded-full"></span>
                <span id="view-created-at" class="block text-gray-500 text-sm mt-2"></span>
            </div>
            <div id="view-attachment" class="mt-4">
                <!-- Attachment will be inserted here via JS -->
            </div>
                ...
                <div class="bottom-content">
                    ...
                    <div class="settings">
                        ...
                        <ul class="menu">
                            ...
                            <li onclick="updateNote(<?php echo $note['journal_id']; ?>, '<?php echo addslashes($note['title']); ?>', '<?php echo addslashes($note['description']); ?>', '<?php echo addslashes($note['mood']); ?>')">
